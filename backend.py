...
def read_text_from_gcs(bucket_name, blob_name):
    bucket = client.get_bucket(bucket_name)
    blob = bucket.blob(blob_name)
    data = blob.download_as_text()  # Download the text file as a string
    return data
text_data = read_text_from_gcs(bucket_name, "Data/14.txt")
text_data2 = read_text_from_gcs(bucket_name, "Data/15.txt")"""
